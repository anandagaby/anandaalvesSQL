...
--
-- Host: 127.0.0.1
-- Tempo de geração: 18/02/2025 às 21:10
...
--
-- Banco de dados: `ananda_financeiro`
--
CREATE DATABASE IF NOT EXISTS `ananda_financeiro` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
USE `ananda_financeiro`;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `mySp_correntistaInsert` (`v_nome` VARCHAR(60), `v_cpf` VARCHAR(20))   BEGIN IF((v_nome!='') && (v_cpf!='')) THEN 
    INSERT INTO tbl_correntista 
    (correntista_nome,correntista_cpf) 
    VALUES 
    (v_nome,v_cpf); 
    ELSE SELECT 'nome e cpf devem ser fornecidos para o cadastro!' 
    AS Msg; 
    END IF; 
    END$$
...
(1, 'Ananda Gabriely Alves', '987.654.432-10'),
(2, 'Arya Stark', '123.456.789-01');
